...
class MailServer(
  ...
  override fun send(message: EmailMessage): Map<String, DeliveryResult> {
    ...
    val sender =
        find(message.from)
            ?: return message.recipients.associateWith {
              DeliveryResult.PermanentFailure("sender not found")
            }

    if (!sender.canSend()) {
      return message.recipients.associateWith {
        DeliveryResult.PermanentFailure("sender not allowed to send")
      }
    }
    ...
    if (totalAttachmentSize > maxTotalAttachmentPerMessage) {
      message.recipients.forEach {
        results[it] = DeliveryResult.TransientFailure("attachments too large")
      }
      return results
    }
