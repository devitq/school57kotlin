...
abstract class Account(val id: String, val owner: Customer, val currency: Currency) {
      ...
      transactions.forEach {
        when (it) {
          ...
          is TransferTransaction ->
              println(
                  "[${it.id}] Перевод: ${it.from.id} -> ${it.to.id}, сумма: ${it.amount}, описание: ${it.description}"
              )
        }
      }
    }
