...
class ExpandOrdersTest {
  ...
  fun `expands orders into line items with tax`() {
    val orders =
        listOf(
            Order(id = "order-1", items = listOf(OrderItem("A", 2, 10.0), OrderItem("B", 1, 5.0))),
            Order(id = "order-2", items = listOf(OrderItem("C", 3, 20.0))),
        )
  ...
  fun `applies custom tax rate`() {
    val orders = listOf(Order("o1", listOf(OrderItem("P", 1, 100.0))))
