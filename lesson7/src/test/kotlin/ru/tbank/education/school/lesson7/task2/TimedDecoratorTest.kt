...
class TimedDecoratorTest {
  ...
  fun `должен измерять и логировать время выполнения`() {
    ...
    try {
      ...
      Assertions.assertTrue(
          duration!! >= 90L,
          "Ожидается, что длительность будет хотя бы 90 мс (sleep 100)",
      )
    } finally {
  ...
  fun `должен корректно работать с любыми типами`() {
    ...
    try {
      ...
      val printed = output.toString().trim()
      Assertions.assertTrue(
          printed.contains("[Concat] выполнено за"),
          "Ожидается вывод таймера в консоль",
      )
    } finally {
      System.setOut(originalOut)
    }
